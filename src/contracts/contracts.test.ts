...
describe('contracts payload validators', () => {
  it('validateHostToGuestPayload: HOST_VISIBILITY', () => {
    ...
    expect(validateHostToGuestPayload('HOST_VISIBILITY', { visible: true, tabId: 't1' })).toBe(true);
  ...
  it('validateGuestToHostPayload: GUEST_SLEEP_STATE', () => {
    ...
    expect(
      validateGuestToHostPayload('GUEST_SLEEP_STATE', { sleeping: false, reason: 'hidden', tabId: 't1' })
    ).toBe(true);
