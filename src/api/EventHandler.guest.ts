...
>(requestEvent: TReqName, responseEvent: TResName) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      pendingRequestsRef.current.forEach((entry, key) => {
        ...
        entry.reject(
          new Error(
            `[useEventBridge] Disposed: ${String(requestEvent)} -> ${String(responseEvent)} (${key})`
          )
        );
      });
  ...
  const request = useCallback(
    async (
      ...
      if (!requestId) {
        throw new Error(`[useEventBridge] requestId is required for ${String(requestEvent)}`);
      }
      ...
      return new Promise<HostToGuestEventPayloads[TResName]>((resolve, reject) => {
        if (isDisposedRef.current) {
          reject(
            new Error(
              `[useEventBridge] Disposed before request: ${String(requestEvent)} -> ${String(responseEvent)} (ID: ${requestId})`
            )
          );
          return;
        }
        ...
        const timer = setTimeout(() => {
          if (pendingRequestsRef.current.has(key)) {
            ...
            reject(
              new Error(
                `[useEventBridge] Timeout: ${String(requestEvent)} -> ${String(responseEvent)} (ID: ${requestId})`
              )
            );
          }
        }, timeoutMs);
